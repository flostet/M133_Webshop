...
import { Cart, Item, Product } from "../common/types.ts";
...
async function loadProducts(): Promise<Product[]> {
  const jsonFile = await Deno.readTextFile(
    `${Deno.cwd()}/src/backend/data/products.json`,
  );
...
router
  ...
  .get("/api/products/:id", async (context) => {
    const index = products.findIndex((p) =>
      p.id.toString() == context.params.id
    );
    if (index >= 0) {
  ...
  .get("/api/images/:image", async (context) => {
    const image = await Deno.readFile(
      `${Deno.cwd()}/src/backend/data/images/${context.params.image}`,
    );
